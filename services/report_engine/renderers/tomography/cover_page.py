from __future__ import annotations

from datetime import datetime
from pathlib import Path
from typing import Any

import fitz

from services.report_engine.layout_engine import ReportLayoutEngine
from services.report_engine.report_context import ReportRenderContext


class TomographyCoverPage:
    """Primeira página do relatório de Tomografia Industrial."""

    COLOR_NAVY = (0.025, 0.110, 0.215)
    COLOR_TEXT = (0.070, 0.100, 0.135)
    COLOR_MUTED = (0.360, 0.410, 0.470)
    COLOR_BORDER = (0.790, 0.825, 0.860)
    COLOR_SURFACE = (0.975, 0.982, 0.988)
    COLOR_LIGHT_BLUE = (0.925, 0.960, 0.987)

    SECTION_TITLE_HEIGHT = 26.0
    GAP = 10.0

    def __init__(self, *, base_dir: Path):
        self.base_dir = base_dir

    def render(
        self,
        *,
        layout: ReportLayoutEngine,
        render_context: ReportRenderContext,
    ) -> None:
        page = layout.ensure_page()
        self._draw_report_identity(
            page=page,
            layout=layout,
            render_context=render_context,
        )
        self._draw_process_and_image(
            page=page,
            layout=layout,
            render_context=render_context,
        )
        self._draw_acquisition_summary(
            layout=layout,
            render_context=render_context,
        )

    def _draw_report_identity(
        self,
        *,
        page: fitz.Page,
        layout: ReportLayoutEngine,
        render_context: ReportRenderContext,
    ) -> None:
        height = 50.0
        page = layout.ensure_space(height)
        rect = layout.full_width_rect(height)

        page.draw_rect(
            rect,
            color=self.COLOR_BORDER,
            fill=(1, 1, 1),
            width=0.6,
        )

        divider_x = rect.x1 - 150

        page.insert_textbox(
            fitz.Rect(
                rect.x0 + 12,
                rect.y0 + 8,
                divider_x - 10,
                rect.y1 - 6,
            ),
            "RELATÓRIO TÉCNICO\nDE TOMOGRAFIA INDUSTRIAL",
            fontsize=11.2,
            fontname="hebo",
            color=self.COLOR_NAVY,
            lineheight=1.05,
        )

        page.draw_line(
            fitz.Point(divider_x, rect.y0 + 7),
            fitz.Point(divider_x, rect.y1 - 7),
            color=self.COLOR_BORDER,
            width=0.5,
        )

        page.insert_textbox(
            fitz.Rect(
                divider_x + 10,
                rect.y0 + 7,
                rect.x1 - 8,
                rect.y0 + 19,
            ),
            "Relatório nº",
            fontsize=6.4,
            fontname="hebo",
            color=self.COLOR_MUTED,
        )

        page.insert_textbox(
            fitz.Rect(
                divider_x + 10,
                rect.y0 + 19,
                rect.x1 - 8,
                rect.y0 + 31,
            ),
            self._clean_text(
                render_context.project.report_id,
                fallback="-",
            ),
            fontsize=8.4,
            fontname="hebo",
            color=self.COLOR_NAVY,
        )

        if render_context.section_enabled("show_version"):
            page.insert_textbox(
                fitz.Rect(
                    divider_x + 10,
                    rect.y0 + 31,
                    rect.x1 - 8,
                    rect.y1 - 5,
                ),
                f"Versão {render_context.project.version or 'V1.0'}",
                fontsize=6.6,
                fontname="helv",
                color=self.COLOR_TEXT,
            )

        layout.advance(height + self.GAP)

    def _draw_process_and_image(
        self,
        *,
        page: fitz.Page,
        layout: ReportLayoutEngine,
        render_context: ReportRenderContext,
    ) -> None:
        objective = self._context_value(
            render_context,
            "inspection_objective",
            "objective",
            "tomography_objective",
        )

        rows = [
            ("Processo", render_context.project.name),
            ("Peça / modelo", render_context.project.part_name),
            ("Código da peça", render_context.project.part_code),
            ("Cliente", render_context.project.client),
            (
                "Equipamento",
                render_context.project.equipment
                or "ZEISS BOSELLO MAX",
            ),
        ]

        if self._has_text(objective):
            rows.append(("Objetivo", objective))

        rows.append(
            (
                "Data de emissão",
                datetime.now().strftime("%d/%m/%Y"),
            )
        )

        row_heights = self._calculate_row_heights(rows)
        content_height = max(218.0, sum(row_heights))
        block_height = self.SECTION_TITLE_HEIGHT + content_height

        page = layout.ensure_space(
            block_height,
            repeated_title="IDENTIFICAÇÃO DA INSPEÇÃO",
        )

        gap = 12.0
        left_width = layout.geometry.content_width * 0.48
        right_width = (
            layout.geometry.content_width
            - left_width
            - gap
        )
        start_y = layout.cursor_y

        left_rect = fitz.Rect(
            layout.geometry.margin_left,
            start_y,
            layout.geometry.margin_left + left_width,
            start_y + block_height,
        )
        right_rect = fitz.Rect(
            left_rect.x1 + gap,
            start_y,
            left_rect.x1 + gap + right_width,
            start_y + block_height,
        )

        self._draw_panel_title(
            page=page,
            rect=left_rect,
            title="1. IDENTIFICAÇÃO DA INSPEÇÃO",
        )
        self._draw_panel_title(
            page=page,
            rect=right_rect,
            title="PEÇA / REGIÃO ANALISADA",
        )

        self._draw_identification_rows(
            page=page,
            rect=fitz.Rect(
                left_rect.x0,
                left_rect.y0 + self.SECTION_TITLE_HEIGHT,
                left_rect.x1,
                left_rect.y1,
            ),
            rows=rows,
            row_heights=row_heights,
        )

        self._draw_primary_image(
            page=page,
            rect=fitz.Rect(
                right_rect.x0,
                right_rect.y0 + self.SECTION_TITLE_HEIGHT,
                right_rect.x1,
                right_rect.y1,
            ),
            render_context=render_context,
        )

        layout.advance(block_height + self.GAP)

    def _draw_acquisition_summary(
        self,
        *,
        layout: ReportLayoutEngine,
        render_context: ReportRenderContext,
    ) -> None:
        parameters = [
            ("Tensão", self._context_value(
                render_context, "voltage", "tube_voltage", "ct_voltage"
            ), "kV"),
            ("Corrente", self._context_value(
                render_context, "current", "tube_current", "ct_current"
            ), "µA"),
            ("Voxel", self._context_value(
                render_context, "voxel_size", "voxel"
            ), ""),
            ("Projeções", self._context_value(
                render_context, "projections", "projection_count"
            ), ""),
            ("Tempo", self._context_value(
                render_context, "scan_duration", "measurement_duration"
            ), ""),
        ]

        parameters = [
            item
            for item in parameters
            if self._has_text(item[1])
        ]

        if not parameters:
            return

        title_height = self.SECTION_TITLE_HEIGHT
        card_height = 68.0
        total_height = title_height + card_height

        page = layout.ensure_space(
            total_height,
            repeated_title="PARÂMETROS DE AQUISIÇÃO",
        )

        title_rect = layout.full_width_rect(title_height)
        self._draw_panel_title(
            page=page,
            rect=title_rect,
            title="2. PARÂMETROS PRINCIPAIS DA AQUISIÇÃO",
        )

        gap = 7.0
        card_width = (
            layout.geometry.content_width
            - gap * (len(parameters) - 1)
        ) / len(parameters)
        start_y = layout.cursor_y + title_height

        for index, (label, value, suffix) in enumerate(parameters):
            x = (
                layout.geometry.margin_left
                + index * (card_width + gap)
            )
            rect = fitz.Rect(
                x,
                start_y,
                x + card_width,
                start_y + card_height,
            )

            page.draw_rect(
                rect,
                color=self.COLOR_BORDER,
                fill=self.COLOR_LIGHT_BLUE,
                width=0.5,
            )
            page.insert_textbox(
                fitz.Rect(
                    rect.x0 + 4,
                    rect.y0 + 7,
                    rect.x1 - 4,
                    rect.y0 + 20,
                ),
                label,
                fontsize=5.6,
                fontname="hebo",
                color=self.COLOR_MUTED,
                align=fitz.TEXT_ALIGN_CENTER,
            )

            display_value = self._clean_text(
                value,
                fallback="-",
            )
            if suffix:
                display_value = f"{display_value} {suffix}"

            page.insert_textbox(
                fitz.Rect(
                    rect.x0 + 4,
                    rect.y0 + 27,
                    rect.x1 - 4,
                    rect.y1 - 8,
                ),
                display_value,
                fontsize=8.5,
                fontname="hebo",
                color=self.COLOR_NAVY,
                align=fitz.TEXT_ALIGN_CENTER,
            )

        layout.advance(total_height + self.GAP)

    def _draw_panel_title(
        self,
        *,
        page: fitz.Page,
        rect: fitz.Rect,
        title: str,
    ) -> None:
        title_rect = fitz.Rect(
            rect.x0,
            rect.y0,
            rect.x1,
            rect.y0 + self.SECTION_TITLE_HEIGHT,
        )
        page.draw_rect(
            title_rect,
            color=self.COLOR_NAVY,
            fill=self.COLOR_NAVY,
            width=0.5,
        )
        page.insert_textbox(
            fitz.Rect(
                title_rect.x0 + 9,
                title_rect.y0 + 6,
                title_rect.x1 - 9,
                title_rect.y1 - 3,
            ),
            title,
            fontsize=7.2,
            fontname="hebo",
            color=(1, 1, 1),
        )

    def _draw_identification_rows(
        self,
        *,
        page: fitz.Page,
        rect: fitz.Rect,
        rows: list[tuple[str, Any]],
        row_heights: list[float],
    ) -> None:
        label_width = rect.width * 0.37
        y = rect.y0

        for index, ((label, value), row_height) in enumerate(
            zip(rows, row_heights)
        ):
            row_rect = fitz.Rect(
                rect.x0,
                y,
                rect.x1,
                y + row_height,
            )
            fill = (
                self.COLOR_SURFACE
                if index % 2 == 0
                else (1, 1, 1)
            )
            page.draw_rect(
                row_rect,
                color=self.COLOR_BORDER,
                fill=fill,
                width=0.35,
            )
            page.insert_textbox(
                fitz.Rect(
                    row_rect.x0 + 8,
                    row_rect.y0 + 7,
                    row_rect.x0 + label_width,
                    row_rect.y1 - 4,
                ),
                label,
                fontsize=6.1,
                fontname="hebo",
                color=self.COLOR_NAVY,
            )
            page.insert_textbox(
                fitz.Rect(
                    row_rect.x0 + label_width,
                    row_rect.y0 + 7,
                    row_rect.x1 - 8,
                    row_rect.y1 - 4,
                ),
                self._clean_text(value),
                fontsize=6.3,
                fontname="helv",
                color=self.COLOR_TEXT,
                lineheight=1.10,
            )
            y += row_height

        if y < rect.y1:
            page.draw_rect(
                fitz.Rect(
                    rect.x0,
                    y,
                    rect.x1,
                    rect.y1,
                ),
                color=self.COLOR_BORDER,
                fill=(1, 1, 1),
                width=0.35,
            )

    def _draw_primary_image(
        self,
        *,
        page: fitz.Page,
        rect: fitz.Rect,
        render_context: ReportRenderContext,
    ) -> None:
        page.draw_rect(
            rect,
            color=self.COLOR_BORDER,
            fill=self.COLOR_SURFACE,
            width=0.5,
        )

        image = render_context.primary_image
        if image is None:
            return

        image_path = Path(
            str(
                getattr(
                    image,
                    "file_path",
                    "",
                )
            )
        )

        caption = self._optional_text(
            getattr(
                image,
                "caption",
                None,
            )
        )
        caption_height = 28.0 if caption else 0.0

        image_rect = fitz.Rect(
            rect.x0 + 10,
            rect.y0 + 10,
            rect.x1 - 10,
            (
                rect.y1 - caption_height - 8
                if caption
                else rect.y1 - 10
            ),
        )

        if image_path.exists():
            try:
                page.insert_image(
                    image_rect,
                    filename=str(image_path),
                    keep_proportion=True,
                )
            except Exception:
                return

        if caption:
            page.insert_textbox(
                fitz.Rect(
                    rect.x0 + 8,
                    rect.y1 - caption_height,
                    rect.x1 - 8,
                    rect.y1 - 6,
                ),
                caption,
                fontsize=5.8,
                fontname="helv",
                color=self.COLOR_MUTED,
                align=fitz.TEXT_ALIGN_CENTER,
            )

    def _context_value(
        self,
        render_context: ReportRenderContext,
        *keys: str,
    ) -> Any:
        parameters = (
            render_context.context.get(
                "tomography_parameters",
                {},
            )
            or {}
        )

        if isinstance(parameters, dict):
            for key in keys:
                value = parameters.get(key)
                if self._has_text(value):
                    return value

        for key in keys:
            value = render_context.context.get(key)
            if self._has_text(value):
                return value

        return None

    def _calculate_row_heights(
        self,
        rows: list[tuple[str, Any]],
    ) -> list[float]:
        result = []
        for _, value in rows:
            text = self._clean_text(value)
            lines = max(
                1,
                (len(text) + 34) // 35,
            )
            result.append(
                max(
                    31.0,
                    min(
                        49.0,
                        18.0 + lines * 9.0,
                    ),
                )
            )
        return result

    def _has_text(self, value: Any) -> bool:
        return bool(str(value or "").strip())

    def _optional_text(
        self,
        value: Any,
    ) -> str | None:
        cleaned = " ".join(str(value or "").split())
        return cleaned or None

    def _clean_text(
        self,
        value: Any,
        *,
        fallback: str = "Não informado",
    ) -> str:
        cleaned = " ".join(str(value or "").split())
        return cleaned or fallback